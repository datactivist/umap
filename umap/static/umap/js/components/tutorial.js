...
class Tutorial {
  ...
  showStep(i) {
    ...
    this.updateStepClass(step ? step.name : null)
    // allow steps that explicitly disable highlighting (noHighlight)
    const target =
      step && step.noHighlight
        ? null
        : step.selector
  ...
  updateStepClass(stepName) {
    const prefix = 'tutorial-step-'
    if (this._currentStepClass) {
      document.body.classList.remove(this._currentStepClass)
      this._currentStepClass = null
    }
    if (stepName) {
      const safeName = String(stepName).replace(/[^a-z0-9-_]/gi, '-')
      const className = `${prefix}${safeName}`
      document.body.classList.add(className)
      this._currentStepClass = className
    }
  }
  ...
  highlightTarget(target) {
    ...
    try {
      ...
      const interactive =
        ...
        target.querySelector('a, button, input, textarea, select, [role="button"]')
      if (interactive) {
        console.log(
          '[Tutorial] target contains interactive children; skipping click proxy'
        )
      ...
      proxy.addEventListener('click', function (e) {
        ...
        try {
          ...
          console.log('[Tutorial] proxy click failed', err)
        }
      })
  ...
  positionPanel(target, step) {
    ...
    const defaultPrev =
      typeof gettext === 'function' ? gettext('Previous') : 'Précédent'
    const defaultNext = typeof gettext === 'function' ? gettext('Next') : 'Suivant'
    const defaultDone =
      typeof gettext === 'function' ? gettext('Done') : 'Passer le tutoriel'
    ...
    const showPrev =
      step && Object.prototype.hasOwnProperty.call(step, 'showPrev')
        ? Boolean(step.showPrev)
        : !isFirst
    if (prevBtn) prevBtn.style.display = showPrev ? '' : 'none'
    // allow per-step override to show/hide the next button
    const showNext =
      step && Object.prototype.hasOwnProperty.call(step, 'showNext')
        ? Boolean(step.showNext)
        : !isLast
    ...
    const titleEl =
      this.panel.querySelector('#tutorial-title') ||
      this.panel.querySelector('.fr-modal__title')
    const textEl =
      this.panel.querySelector('.fr-modal__text') ||
      this.panel.querySelector('.fr-modal__content')
    ...
    if (!anchor.startsWith('corner-')) {
      left = Math.min(
        Math.max(left, window.scrollX + pad),
        window.scrollX + Math.max(vw - panelW - pad, pad)
      )
      top = Math.min(
        Math.max(top, window.scrollY + pad),
        window.scrollY + Math.max(vh - panelH - pad, pad)
      )
    }
  ...
  clearHighlight() {
    for (const el of document.querySelectorAll(
      '.tutorial-highlight, .tutorial-target'
    )) {
